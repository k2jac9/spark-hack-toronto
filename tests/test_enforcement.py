"""Fit C — Enforcement lens (automated traffic-enforcement device density, advisory display overlay).

The lens lifts a severity-weighted automated-enforcement (red-light + speed camera) density onto the
substrate as its OWN static ``enforcement`` overlay. It is DISPLAY-ONLY and ADDITIVE: read-only on
the crowd fields (``load``/``congestion``/``risk``), declares NO levers, carries NO cost, and lives
in ``extra_display_lenses`` (excluded from the optimizer's ``J``) — so it can never move a headline
number. These tests pin the honesty invariants (ADR-0039):

1. bare construction is inert (offline-safe no-op);
2. the static enforcement field is baked at NON-SINK nodes only and normalised 0..1;
...
``enforcement_by_node`` adapter is not wired into this branch yet, so its proximity-fusion +
...
from urbanos.kernel.lenses.enforcement import PROVENANCE, EnforcementLens


# --- a hand-built {node_id: value} map over a substrate's ids (mirrors a baked overlay) ---
def _enforcement_map(sub: Substrate) -> dict[str, float]:
    """A deterministic per-node enforcement map over the substrate ids (sinks included; the lens
    drops them). Avoids the unwired ``enforcement_by_node`` adapter — the lens is tested
    directly."""
...
def test_bare_lens_is_inert():
    sub = _toy_substrate()
    lens = EnforcementLens()
    lens.configure(sub)
    st = _state(sub)
    lens.couple(st, 0.0)
    assert "enforcement" not in st.fields            # wrote no overlay
    assert lens.observe(st, 0.0) == {}               # no metric when inert


def test_bare_lens_inert_in_a_full_run():
    sc = downtown_scenario()
    stack = [EventSurge(events=sc.events), EconomicLens(), EnforcementLens()]
    res = Simulation(sc.substrate, stack, params={"release_minutes": 0.0}, dt=sc.dt).run(
        sc.horizon
    )
    assert res.series("enforcement_peak") == []      # inert -> no metric


# --- 2. static field baked at non-sink nodes only; normalised ----------------
def test_couple_writes_normalised_overlay_at_non_sinks_only():
    """The enforcement overlay carries the configured value at the non-sink node (normalised to
    the peak), the sink's value is IGNORED, and the field is the SAME every step (static)."""
    sub = _toy_substrate()
    enforcement = {"a": 150.0, "s": 999.0}           # sink value must be ignored
    lens = EnforcementLens(enforcement)
    lens.configure(sub)
    ai, si = sub.idx("a"), sub.idx("s")
    st = _state(sub)
    lens.couple(st, 0.0)
    overlay = st.fields["enforcement"]
    assert overlay[ai] == pytest.approx(1.0)         # the only node -> peak -> normalised 1.0
    assert overlay[si] == 0.0                         # sink never seeded
    assert lens.observe(st, 0.0)["enforcement_peak"] == pytest.approx(1.0)
    # static: a later step writes the identical field
    st2 = _state(sub)
    lens.couple(st2, 99.0)
    assert np.array_equal(st.fields["enforcement"], st2.fields["enforcement"])


def test_relative_shape_is_normalised():
    """Two non-sink nodes keep their RELATIVE enforcement coverage after normalisation (the shape
    is the claim): the more-enforced node is 1.0, the less-enforced is its ratio."""
    g = nx.DiGraph()
    g.add_node("a", lat=43.60, lng=-79.40, capacity=100.0)
    ...
    lens = EnforcementLens({"a": 200.0, "b": 50.0})
    lens.configure(sub)
    st = _state(sub)
    lens.couple(st, 0.0)
    ov = st.fields["enforcement"]
...
def test_no_nan_or_inf_from_degenerate_inputs():
    sub = _toy_substrate()
    lens = EnforcementLens({"a": float("nan")})      # all degenerate
    lens.configure(sub)
    st = _state(sub)
    lens.couple(st, 0.0)                              # must not raise
    assert np.all(np.isfinite(st.fields["enforcement"]))
    assert st.fields["enforcement"][sub.idx("a")] == 0.0   # nan dropped -> nothing seeded


def test_empty_series_does_not_raise():
    sub = _toy_substrate()
    for series in ({}, None):
        lens = EnforcementLens(series)
        lens.configure(sub)
        st = _state(sub)
        lens.couple(st, 0.0)                          # inert, no raise
        assert "enforcement" not in st.fields


# --- 3. no levers, zero cost, provenance; bounded exposure -------------------
def test_no_levers_no_cost_and_provenance():
    sub = _toy_substrate()
    lens = EnforcementLens({"a": 10.0})
    lens.configure(sub)
    assert lens.levers() == []
...
def test_crush_enforcement_exposure_is_bounded():
    """The advisory exposure overlap is a scale-free cosine in [0, 1] (display-only)."""
    sub = _toy_substrate()
    lens = EnforcementLens({"a": 10.0})
    lens.configure(sub)
    st = _state(sub)
    st.fields["load"][sub.idx("a")] = 5.0            # crowd coincides with enforcement at 'a'
    lens.couple(st, 0.0)
    m = lens.observe(st, 0.0)
    assert 0.0 <= m["crush_enforcement_exposure"] <= 1.0
    assert m["crush_enforcement_exposure"] == pytest.approx(1.0)   # perfectly aligned profiles


# --- 4. read-only on the crowd fields (the additivity contract) --------------
def test_lens_does_not_perturb_crowd_fields_or_economic_terms():
    sc = downtown_scenario()
    enforcement = _enforcement_map(sc.substrate)     # hand-built map (adapter unwired)

    def run(with_lens: bool):
        stack = [EventSurge(events=sc.events), EconomicLens()]
        if with_lens:
            stack.append(EnforcementLens(enforcement))
        return Simulation(
...
def test_deterministic():
    ...
    def run():
        stack = [EventSurge(events=sc.events), EconomicLens(), EnforcementLens(enforcement)]
        return Simulation(
            sc.substrate, stack, params={"release_minutes": 6.0}, dt=sc.dt
        ).run(sc.horizon)

    a, b = run(), run()
    assert a.series("crush_enforcement_exposure") == b.series("crush_enforcement_exposure")
    assert np.allclose(a.frames[-1]["load"], b.frames[-1]["load"])


# --- adapter: proximity fusion + synthetic fallback (wired post-integration) --
def test_enforcement_by_node_concentrates_density_near_points():
    from urbanos.kernel.adapters import enforcement_by_node
    ...
    rr = enforcement_by_node(sub, provider=lambda: pts)
    assert rr["a"] > rr["b"]
    assert rr["s"] == 0.0
    assert all(np.isfinite(v) for v in rr.values())


def test_enforcement_by_node_synthetic_fallback():
    from urbanos.kernel.adapters import enforcement_by_node

    sc = downtown_scenario()
    rr = enforcement_by_node(sc.substrate, provider=lambda: [])   # empty -> synthetic
    sub = sc.substrate
...
def test_enforcement_by_node_default_provider_is_offline_safe():
    from urbanos.kernel.adapters import enforcement_by_node, reset_enforcement_cache

    reset_enforcement_cache()
    sc = downtown_scenario()
    rr = enforcement_by_node(sc.substrate)            # default provider
