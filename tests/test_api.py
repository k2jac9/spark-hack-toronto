...
def _row(addrs: list[dict], address: str) -> dict:
    ...
    return next(a for a in addrs if normalize_address(a["label"]) == key)
...
def test_endpoints_with_fixtures():
    with TestClient(app) as client:
        ...
        # Two-index pins (ADR 0014): both scores + both bands, never a blended score.
        hot = _row(addrs, "100 Queen St W")
        assert hot["risk_safety"] == 0.593 and hot["band_safety"] == "medium"
        assert hot["risk_activity"] == 0.113 and hot["band_activity"] == "low"
        assert "risk_score" not in hot  # no single blended score leaks through
        clean = _row(addrs, "55 John St")
        assert clean["risk_safety"] == 0.0 and clean["risk_activity"] == 0.0

        analyze = client.get("/analyze", params={"address": "100 Queen St W"}).json()
        assert analyze["risk_safety"] == 0.593 and analyze["band_safety"] == "medium"
        assert analyze["risk_activity"] == 0.113 and analyze["band_activity"] == "low"
        assert analyze["found"] is True
        assert "risk_score" not in analyze and "risk_band" not in analyze
        ...
        assert missing["risk_safety"] == 0.0 and missing["risk_activity"] == 0.0
