...
| 2:45 | "It's all **local on the GX10**: **Nemotron-3-Nano** for interactive narration — warm in **under two seconds**, served behind **NVIDIA TensorRT-LLM** for the fastest decode on the box — and a larger MoE Nemotron for batch digests. We deliberately chose **small-active MoE models** for the box's memory bandwidth, and built **ARM64 / aarch64** end to end." | Split-screen `make llm-check`: **TensorRT-LLM vs Ollama decode tok/s** (the live speedup number). Then back to live UI. | Best-use-of-Nemotron + ARM + the **one real on-camera speedup** (ADR-0027). ⚠️ only say "TensorRT-LLM" if the box engine is built & the tok/s is on screen. |
